...
class TaskRunner:
    ...
    def process_file(self, path: Path, report_path: Path) -> None:
        ...
        try:
            ...
            def pc(v):
                return (
                    round((v / self.candle_count * 100), 2)
                    if self.candle_count > 0
                    else 0
                )

            def prob(curr, prev):
                return round((curr / prev * 100), 2) if prev > 0 else 0

            def ch_pc(v):
                return (
                    round((v / self.color_changes * 100), 2)
                    if self.color_changes > 0
                    else 0
                )

            def ratio(v, t):
                return round((v / t * 100), 2) if t > 0 else 0

            res = {
                "file": path.name,
                "candles": self.candle_count,
                "ch_t": self.color_changes,
                "ch_%": pc(self.color_changes),
                "ch_g": self.green_changes,
                "ch_g_%": ch_pc(self.green_changes),
                "ch_r": self.red_changes,
                "ch_r_%": ch_pc(self.red_changes),
                "p1_c": prob(f2, self.color_changes),
                "p1_r": (
                    round(100 - prob(f2, self.color_changes), 2)
                    if self.color_changes > 0
                    else 0
                ),
                "p2_c": prob(f3, f2),
                "p2_r": round(100 - prob(f3, f2), 2) if f2 > 0 else 0,
                "p3_c": prob(f4, f3),
                "p3_r": round(100 - prob(f4, f3), 2) if f3 > 0 else 0,
                "p4_c": prob(f5, f4),
                "p4_r": round(100 - prob(f5, f4), 2) if f4 > 0 else 0,
                "p5_c": prob(f6, f5),
                "p5_r": round(100 - prob(f6, f5), 2) if f5 > 0 else 0,
                "f2_t": f2,
                "f2_%": pc(f2),
                "f2_g": self.g_follows_2,
                "f2_g_%": ratio(self.g_follows_2, f2),
                "f2_r": self.r_follows_2,
                "f2_r_%": ratio(self.r_follows_2, f2),
                "f3_t": f3,
                "f3_%": pc(f3),
                "f3_g": self.g_follows_3,
                "f3_g_%": ratio(self.g_follows_3, f3),
                "f3_r": self.r_follows_3,
                "f3_r_%": ratio(self.r_follows_3, f3),
                "f4_t": f4,
                "f4_%": pc(f4),
                "f4_g": self.g_follows_4,
                "f4_g_%": ratio(self.g_follows_4, f4),
                "f4_r": self.r_follows_4,
                "f4_r_%": ratio(self.r_follows_4, f4),
                "f5_t": f5,
                "f5_%": pc(f5),
                "f5_g": self.g_follows_5,
                "f5_g_%": ratio(self.g_follows_5, f5),
                "f5_r": self.r_follows_5,
                "f5_r_%": ratio(self.r_follows_5, f5),
                "f6_t": f6,
                "f6_%": pc(f6),
                "f6_g": self.g_follows_6,
                "f6_g_%": ratio(self.g_follows_6, f6),
                "f6_r": self.r_follows_6,
                "f6_r_%": ratio(self.r_follows_6, f6),
            }
