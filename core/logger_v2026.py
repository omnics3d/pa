...
DEFAULT_FORMAT: Final = "%(asctime)s | %(levelname)-8s | %(name)s | %(message)s"
COLOR_FORMAT: Final = "%(log_color)s" + DEFAULT_FORMAT + "%(reset)s"
...
class SafeJsonFormatter(jsonlogger.JsonFormatter if _HAS_JSON else object):
...
def setup_logging(
    ...
    if log_files:
        ...
        for lvl_key, path in log_files.items():
            ...
            h = handler_cls(
                str(log_path), "a", MAX_LOG_SIZE, BACKUP_COUNT, encoding="utf-8"
            )
            h.addFilter(SpecificLevelFilter(lvl_int))

            if json_format and _HAS_JSON:
                h.setFormatter(
                    SafeJsonFormatter("%(asctime)s %(levelname)s %(name)s %(message)s")
                )
            else:
