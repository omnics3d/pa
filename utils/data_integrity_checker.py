...
class DataIntegrityChecker:
    ...
    def check_all_files(self):
        ...
        for i, file_path in enumerate(files, 1):
            ...
            progress_lines = info_lines + [
                f"Проверка файла {i}/{self.file_count}: {file_name}"
            ]
    ...
    def _check_single_file(self, file_path: str):
        ...
        try:
            ...
            if not rows:
                self.errors.append(
                    f"Файл: {file_name} - Файл пуст или содержит только заголовки"
                )
                self.total_errors += 1
                return
            ...
            for i, row in enumerate(rows):
                if len(row) != 7:
                    self.errors.append(
                        f"Файл: {file_name}, строка {i+1} - Неверное количество колонок: {len(row)} вместо 7"
                    )
                    self.total_errors += 1
                    continue
                ...
                try:
                    ...
                    self.errors.append(
                        f"Файл: {file_name}, строка {i+1} - Ошибка преобразования данных: {e}"
                    )
                    self.total_errors += 1
    ...
    def _check_timeline(self, data: list, file_name: str, file_date: datetime.date):
        ...
        if out_of_order_count > 0:
            self.errors.append(
                f"Файл: {file_name} - Нарушен хронологический порядок: {out_of_order_count} нарушений"
            )
            self.total_errors += 1

        if wrong_date_count > 0:
            self.errors.append(
                f"Файл: {file_name} - Найдены строки с неправильной датой (UTC): {wrong_date_count} строк"
            )
            self.total_errors += 1
        # }}}
    ...
    def _check_time_gaps(self, data: list, file_name: str, file_date: datetime.date):
        ...
        if gap_count > 0:
            self.errors.append(
                f"Файл: {file_name} - Найдены пропуски: {gap_count} пропусков, максимальный: {max_gap:.1f} мин"
            )
            self.total_errors += 1
        # }}}
    ...
    def _check_price_integrity(self, data: list, file_name: str):
        ...
        if errors_found["negative"] > 0:
            error_messages.append(
                f"отрицательные/нулевые цены: {errors_found['negative']} строк"
            )

        if error_messages:
            self.errors.append(
                f"Файл: {file_name} - Ошибки цен: {', '.join(error_messages)}"
            )
            self.total_errors += 1
        # }}}
    ...
    def _check_volume_integrity(self, data: list, file_name: str):
        ...
        if errors_found["negative_volume"] > 0:
            error_messages.append(
                f"отрицательный volume: {errors_found['negative_volume']} строк"
            )
        if errors_found["negative_turnover"] > 0:
            error_messages.append(
                f"отрицательный turnover: {errors_found['negative_turnover']} строк"
            )
        if errors_found["zero_volume_nonzero_turnover"] > 0:
            error_messages.append(
                f"нулевой volume с ненулевым turnover: {errors_found['zero_volume_nonzero_turnover']} строк"
            )

        if error_messages:
            self.errors.append(
                f"Файл: {file_name} - Ошибки объемов: {', '.join(error_messages)}"
            )
            self.total_errors += 1
        # }}}

    def _check_daily_completeness(
        self, data: list, file_name: str, file_date: datetime.date
    ):
        # {{{
        ...
        if actual_rows < min_expected_rows:
            ...
            self.errors.append(
                f"Файл: {file_name} - Недостаточно данных: {actual_rows} строк вместо {min_expected_rows} (пропущено {missing} минут)"
            )
            self.total_errors += 1
        elif actual_rows > min_expected_rows:
            extra = actual_rows - min_expected_rows
            self.errors.append(
                f"Файл: {file_name} - Избыток данных: {actual_rows} строк вместо {min_expected_rows} (лишних {extra} записей)"
            )
            self.total_errors += 1
        ...
        expected_first = datetime(
            file_date.year, file_date.month, file_date.day, 0, 0, 0, tzinfo=timezone.utc
        )
        ...
        if first_dt != expected_first:
            self.errors.append(
                f"Файл: {file_name} - Первая свеча не в 00:00 UTC: {first_time_str} UTC"
            )
            self.total_errors += 1

        if last_dt != expected_last:
            self.errors.append(
                f"Файл: {file_name} - Последняя свеча не в 23:59 UTC: {last_time_str} UTC"
            )
            self.total_errors += 1
        ...
        if wrong_day_count > 0:
            self.errors.append(
                f"Файл: {file_name} - Свечи не в том дне (UTC): {wrong_day_count} строк"
            )
            self.total_errors += 1
        # }}}
