...
class DataLoader:
    ...
    def get_candles(
        ...
        search_end_dt = (
            offset_date if offset_date else datetime.strptime(end_date_str, "%Y-%m-%d")
        )
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    if not t_str.replace(".", "", 1).isdigit():
                        continue
                    try:
                        t_val = int(float(t_str))
                        if t_val > 2000000000:
                            t_val //= 1000  # Коррекция ms -> s
                        data.append(
                            {
                                "t": t_val,
                                "o": float(row[1]),
