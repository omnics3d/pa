...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        xml_path = os.path.join(base_dir, "storage", "state", 
                                "exchanges_config.xml")
        self.manager = (ExchangeDataManager(xml_path) if ExchangeDataManager 
                        and os.path.exists(xml_path) else None)
    ...
    def _update_chart(self):
        self.clear_layout()
        title = (f"{self.current_tool} > {self.current_tool}" 
                 if self.current_tool else "Chart")
    ...
    def load_more_history(self):
        ...
        new_data = self.loader.get_candles(s_date, "", 
                                          timeframe_min=self.current_tf, 
                                          limit=1000, 
    ...
    def custom_tf_multiplier(self, base_minutes: int, label: str):
        val, ok = QInputDialog.getInt(self, "Timeframe", 
                                      f"Enter multiplier for {label}:", 
                                      1, 1, 100000)
        if ok:
