...
import argparse
import requests
from lib.exchange_config_loader import (check_the_full_name_of_the_exchange,
...
class SetDownloadOptions:
    ...
    def __check_market_param(self, exchange:str, market: str):
        ...
        if check_the_full_name_of_the_market(exchange, market):
            ...
            name_market_list: list[str] = (
                    get_full_names_of_all_markets(exchange))
            name: str
    ...
    def __check_tool_param(self, exchange: str, market: str, tool: str):
       #{{{
        ...
        if check_the_full_name_of_the_tool(exchange, market, tool):
            ...
            name_tool_list: list[str] = (
                    get_names_all_tools(exchange, market))
            name: str
...
class LoadingBayBit:
    ...
    def __set_start_block_day(self):
        ...
        self.start_datetime = datetime.strptime(
                start_datetime, '%Y-%m-%d %H:%M:%S').replace(
                        tzinfo=timezone.utc)
    ...
    def __load_block(self):
        ...
        param = {'category': self.SDO.market, 'symbol': self.SDO.tool,
                 'interval': '1', 'start': start, 'end': end, 'limit': 180}
    ...
    def __save_day(self):
        ...
        """
        name_file: str = (
                self.SDO.tool + '-1m-' + self.SDO.start_date_load + '.csv')
        path: str = self.SDO.path_save + name_file
        with open(path, 'w') as file:
            file.write(self.candle_list)
        save_end_date_load(
                self.SDO.exchange, self.SDO.market,
                self.SDO.tool, self.SDO.start_date_load,)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--exchange", type=str, default="bybit",
                        help="Биржа с которой будет производится загрузка.")
    parser.add_argument("-m", "--market", type=str, default="linear",
                        help="Рынок, с которого будет производится загрузка.")
    parser.add_argument("-t", "--tool", type=str, default="DOGEUSDT",
                        help="Инструмент для загрузки.")
    agrs: argparse.Namespace =  parser.parse_args()
    #}}}
    LB = LoadingBayBit(exchange = agrs.exchange, market = agrs.market,
                       tool = agrs.tool)
    LB.start_load()
